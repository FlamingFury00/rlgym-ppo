...
def _form_printable_groups(report):
    ...
    groups = [
        ...
        },
        {
            ...
            "Value Function Update Magnitude": report[
                "Value Function Update Magnitude"
            ],
        },
    ]

    # Muesli-specific metrics (only show if they exist)
    muesli_metrics = {
        key: report[key]
        for key in ["Model Loss", "Reward Loss", "Total Loss", "Policy Loss"]
        if key in report
    }
    if muesli_metrics:
        groups.append(muesli_metrics)
